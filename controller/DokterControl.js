...
exports.diagnosa = async (req, res) => {
  ...
  const sedangAntri = await antrian.findOne({
    where: { antrian_id: antrianId },
  });
  const data = req.body.pilihObat;
  if (!sedangAntri)
    return res.status(404).json({ alert: "Pasien sudah di diagnosa" });
  diagnosa
    .create({
    ...
    .then((newDiagnosis) => {
      if (!newDiagnosis)
        return res.status(500).json({ alert: "Gagal membuat diagnosa pasien" });
...
exports.sseDiagnosis = async (req, res) => {
  ...
  const handleNewDiagnosis = async () => {
    res.write(
      `data: ${await getLastDiagnosis(namaKlinik, hari, dokterid)}\n\n`
    );
  };
